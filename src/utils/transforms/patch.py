...
from ._base import _index_select_data, register_transform, _get_CB_positions, _get_CA_positions
...
@register_transform('selected_interface_patch')
class SelectedInterfacePatch(object):
    ...
    def __call__(self, data):
        ...
        def select_residues(chain_idx):

            dist_from_interface = torch.cdist(pos_CA[chain_idx], pos_CA[interface_idx]).min(dim=1)[0]

            sorted_idx = torch.argsort(dist_from_interface)
            chain_patch_idx = chain_idx[sorted_idx[:self.patch_size]]

            return chain_patch_idx

        chain0_idx = torch.where(data.chain_nb == 0)[0]
        chain1_idx = torch.where(data.chain_nb == 1)[0]

        interface_idx = torch.where(data.interface_mask)[0]
        pos_CA = _get_CA_positions(data['pos_atoms'])

        chain0_patch_idx = select_residues(chain0_idx)
        chain1_patch_idx = select_residues(chain1_idx)

        patch_idx = torch.cat([chain0_patch_idx, chain1_patch_idx], dim=-1)
        data_patch = _index_select_data(data, patch_idx)

        return data_patch

@register_transform('random_ppiref_patch')
class RandomPPIRefPatch(object):
    ...
    def __call__(self, data):
        ...
        def select_residues(chain_idx):
            if len(chain_idx) <= self.patch_size:
                return chain_idx
            else:
                index_list = list(range(len(chain_idx)))
                random.shuffle(index_list)
                return chain_idx[index_list[:self.patch_size]]
